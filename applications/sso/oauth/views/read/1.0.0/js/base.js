...
window.elements.main.className = 'widget-infinite-enable-print';
    ...
    widgets.oauth = new Infinite();
    widgets.oauth.setOptionSetting(window.page.user.setting.oauth);
    widgets.oauth.setOptionStructure(window.page.tables.oauth.fields);
    widgets.oauth.setContainer(window.elements.content);
    widgets.oauth.setRequestUrl('/api/sso/oauth/read');
    widgets.oauth.setResponseKey('data');
    widgets.oauth.setResponseUnique('_key');
    widgets.oauth.getNotice().setTextEmpty(window.page.getTranslate('infinite.no_result'));
    widgets.oauth.request();
    widgets.oauth.addEventSelect(new Infinite.Event(Infinite.Event.always(), function () {
        window.choosed = this.getTR().getBody().getChecked();
        window.negotiateButtonStatus.call();
    }));

    wrapper.appendChild(widgets.oauth.out());

    if (window.page.checkPolicy('sso/oauth/action/create')) {
        let add = new Button();
        add.addStyle('flat');
        add.getIcon().set('person_add');
        add.setText(window.page.getTranslate('nav.buttons.add'));
        add.onClick(function () {
            window.location = '/sso/oauth/upsert';
        });
        window.buttons.push(add);
    }

    if (window.page.checkPolicy('sso/oauth/action/update')) {
        let edit = new Button();
        edit.addStyle('flat');
        ...
        edit.onClick(function () {
            window.location = '/sso/oauth/upsert/' + window.choosed[0];
        });
    ...
    for (let item = 0; item < window.page.tables.oauth_delete.fields.length; item++) widgets.delete.addInput(window.page.tables.oauth_delete.fields[item]);
    ...
    widgets.modal.buttons.submit.onClick(function () {
        let preloader = widgets.oauth.getPreloader().status();
        ...
        for (let x in window.choosed) {
            ...
            xhr.setCallbackSuccess(function () {
                ...
                widgets.oauth.getBody().removeTR(key);
    ...
    if (window.page.checkPolicy('sso/oauth/action/delete')) {
    ...
    let infinite_setting = widgets.oauth.getSetting();
    ...
    if (infinite_setting !== null && window.page.checkPolicy('iam/user/action/save/widget/setting')) {
        ...
        infinite_setting.setHardcode('widget', 'oauth');
