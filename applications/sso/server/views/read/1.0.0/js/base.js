...
window.elements.main.className = 'widget-infinite-enable-print';
    ...
    widgets.server = new Infinite();
    widgets.server.setOptionSetting(window.page.user.setting.server);
    widgets.server.setOptionStructure(window.page.tables.server.fields);
    widgets.server.setContainer(window.elements.content);
    widgets.server.setRequestUrl('/api/sso/server/read');
    widgets.server.setResponseKey('data');
    widgets.server.setResponseUnique('_key');
    widgets.server.getNotice().setTextEmpty(window.page.getTranslate('infinite.no_result'));
    widgets.server.request();
    widgets.server.addEventSelect(new Infinite.Event(Infinite.Event.always(), function () {
        window.choosed = this.getTR().getBody().getChecked();
        window.negotiateButtonStatus.call();
    }));

    wrapper.appendChild(widgets.server.out());

    if (window.page.checkPolicy('sso/server/action/create')) {
        let add = new Button();
        add.addStyle('flat');
        add.getIcon().set('person_add');
        add.setText(window.page.getTranslate('nav.buttons.add'));
        add.onClick(function () {
            window.location = '/sso/server/upsert';
        });
        window.buttons.push(add);
    }

    if (window.page.checkPolicy('sso/server/action/update')) {
        let edit = new Button();
        edit.addStyle('flat');
        ...
        edit.onClick(function () {
            window.location = '/sso/server/upsert/' + window.choosed[0];
        });
    ...
    for (let item = 0; item < window.page.tables.server_delete.fields.length; item++) widgets.delete.addInput(window.page.tables.server_delete.fields[item]);
    ...
    widgets.modal.buttons.submit.onClick(function () {
        let preloader = widgets.server.getPreloader().status();
        ...
        for (let x in window.choosed) {
            ...
            xhr.setCallbackSuccess(function () {
                ...
                widgets.server.getBody().removeTR(key);
    ...
    if (window.page.checkPolicy('sso/server/action/delete')) {
    ...
    let infinite_setting = widgets.server.getSetting();
    ...
    if (infinite_setting !== null && window.page.checkPolicy('iam/user/action/save/widget/setting')) {
        ...
        infinite_setting.setHardcode('widget', 'server');
