(function (window) {
    ...
    for (let item = 0; item < window.page.tables.server.fields.length; item++) {
        if (window.page.tables.server.fields[item].name === 'cidr') window.page.tables.server.fields[item][Form.Tooltip.attribute()] = window.page.getTranslate('tooltip.cidr');
        widgets.form.addInput(window.page.tables.server.fields[item]);
    }
    ...
    let create_api = '/api/sso/server/create'
        + String.fromCharCode(63)
        + 'timestamp'
        + String.fromCharCode(61)
        + Date.now();
    ...
    let update_api = '/api/sso/server/update'
    ...
    let main = new WXmlHttpRequest(),
        detail_api = '/api/sso/server/detail'
            + String.fromCharCode(47)
            + encodeURIComponent(window.reference[0])
            + String.fromCharCode(63)
            + 'timestamp'
    ...
    main.setCallbackSuccess(function (response) {
        for (let item in response.data) switch (item) {
            case 'application':
                let map = response.data[item].map(function (value) {
                    return { _key: value }
                });
                widgets.form.set(item, map);
                break;
            default:
