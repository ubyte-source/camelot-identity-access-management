(function (window) {
    ...
    Form.Plugin.Chips.Search.placeholder = function () {
        return window.page.getTranslate('chips.search.no_result');
    }
    ...
    window.prepareProtected = function () {
        for (let item = 0; item < window.page.tables.application.fields.length; item++) {
            if (window.page.tables.application.fields[item].name != 'share'
                && window.page.tables.application.fields[item].name != 'cluster') continue;

            widgets.form.addInput(window.page.tables.application.fields[item]);
        }
    }

    for (let item = 0; item < window.page.tables.application.fields.length; item++) {
        if (window.page.tables.application.fields[item].name === 'icon') window.page.tables.application.fields[item].construct = function () {
    ...
    let create_api = '/api/sso/application/create'
        + String.fromCharCode(63)
        + 'timestamp'
        + String.fromCharCode(61)
        + Date.now();
    ...
    if (window.reference.length === 0) {
        ...
        let update_api = '/api/sso/application/update'
        ...
        let main = new WXmlHttpRequest(),
            detail_api = '/api/sso/application/detail'
                + String.fromCharCode(47)
                + encodeURIComponent(window.reference[0])
                + String.fromCharCode(63)
                + 'timestamp'
        ...
        main.setCallbackSuccess(function (response) {
            if (response.data.hasOwnProperty('owner')
                && response.data.owner.hasOwnProperty('_key')
                && response.data.owner._key === window.page.user._key) window.prepareProtected();
