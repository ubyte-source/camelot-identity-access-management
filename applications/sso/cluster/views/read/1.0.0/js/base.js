...
window.elements.main.className = 'widget-infinite-enable-print';
    ...
    for (let item = 0; item < window.page.tables.cluster.fields.length; item++) switch (window.page.tables.cluster.fields[item].name) {
        case 'label':
            window.page.tables.cluster.fields[item].handling = function (array) {
                let mapped = {}, text = Infinite.Plugin.Text.void();
                ...
                for (let item = 0; item < check.length; item++) if (mapped.hasOwnProperty(check[item])) {
                    text = mapped[check[item]];
                    break;
                }

                let result = document.createElement('div'), node = document.createTextNode(text);
                result.className = 'result';
                result.appendChild(node);
                if (text === Infinite.Plugin.Text.void()) result.classList.add('null');
                return result;
            }
            break;
    }

    widgets.cluster = new Infinite();
    widgets.cluster.setOptionSetting(window.page.user.setting.cluster);
    widgets.cluster.setOptionStructure(window.page.tables.cluster.fields);
    widgets.cluster.setContainer(window.elements.content);
    widgets.cluster.setRequestUrl('/api/sso/cluster/read');
    widgets.cluster.setResponseKey('data');
    widgets.cluster.setResponseUnique('_key');
    widgets.cluster.getNotice().setTextEmpty(window.page.getTranslate('infinite.no_result'));
    widgets.cluster.request();
    widgets.cluster.addEventSelect(new Infinite.Event(Infinite.Event.always(), function () {
        window.choosed = this.getTR().getBody().getChecked();
        window.negotiateButtonStatus.call();
    }));

    wrapper.appendChild(widgets.cluster.out());

    if (window.page.checkPolicy('sso/cluster/action/create')) {
        let add = new Button();
        add.addStyle('flat');
        add.getIcon().set('add');
        add.setText(window.page.getTranslate('nav.buttons.add'));
        add.onClick(function () {
            window.location = '/sso/cluster/upsert';
        });
        window.buttons.push(add);
    }

    if (window.page.checkPolicy('sso/cluster/action/update')) {
        let edit = new Button();
        edit.addStyle('flat');
        ...
        edit.onClick(function () {
            window.location = '/sso/cluster/upsert/' + window.choosed[0];
        });
        window.buttons.push(edit);
    }
    widgets.delete = new Form();
    widgets.delete.getFormElement().setAttribute('data-form-name', 'delete');
    for (let item = 0; item < window.page.tables.cluster_delete.fields.length; item++) widgets.delete.addInput(window.page.tables.cluster_delete.fields[item]);
    ...
    widgets.modal.buttons.submit.onClick(function () {
        let preloader = widgets.cluster.getPreloader().status();
        ...
        for (let x in window.choosed) {
            ...
            xhr.setCallbackSuccess(function () {
                ...
                widgets.cluster.getBody().removeTR(key);
    ...
    if (window.page.checkPolicy('sso/cluster/action/delete')) {
    ...
    let infinite_setting = widgets.cluster.getSetting();
    ...
    if (infinite_setting !== null && window.page.checkPolicy('iam/user/action/save/widget/setting')) {
        ...
        infinite_setting.setHardcode('widget', 'cluster');
    ...
    if (window.page.checkPolicy('iam/user/action/save/widget/setting')) {
        ...
        window.setting.onClick(function (event) {
            ...
            window.setting.getText(window.page.getTranslate('nav.buttons.show_settings'));
        });
