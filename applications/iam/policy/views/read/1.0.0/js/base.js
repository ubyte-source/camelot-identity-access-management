...
window.elements.main.className = 'widget-infinite-enable-print';
    ...
    widgets.policy = new Infinite();
    widgets.policy.setOptionSetting(window.page.user.setting.policy);
    widgets.policy.setOptionStructure(window.page.tables.policy.fields);
    widgets.policy.setContainer(window.elements.content);
    widgets.policy.setRequestUrl('/api/iam/policy/read');
    widgets.policy.setResponseKey('data');
    widgets.policy.setResponseUnique('_key');
    widgets.policy.getNotice().setTextEmpty(window.page.getTranslate('infinite.no_result'));
    widgets.policy.request();
    widgets.policy.addEventSelect(new Infinite.Event(Infinite.Event.always(), function () {
        window.choosed = this.getTR().getBody().getChecked();
        window.negotiateButtonStatus.call();
    }));

    wrapper.appendChild(widgets.policy.out());

    if (window.page.checkPolicy('iam/policy/action/create')) {
        let add = new Button();
        add.addStyle('flat');
        add.getIcon().set('add');
        add.setText(window.page.getTranslate('nav.buttons.add'));
        add.onClick(function () {
            window.location = '/iam/policy/upsert';
        });
        window.buttons.push(add);
    }

    if (window.page.checkPolicy('iam/policy/action/update')) {
        let edit = new Button();
        edit.addStyle('flat');
        ...
        edit.onClick(function () {
            window.location = '/iam/policy/upsert/' + window.choosed[0];
        });
    ...
    for (let item = 0; item < window.page.tables.policy_delete.fields.length; item++) widgets.delete.addInput(window.page.tables.policy_delete.fields[item]);
    ...
    widgets.modal.buttons.submit.onClick(function () {
        let preloader = widgets.policy.getPreloader().status();
        ...
        for (let x in window.choosed) {
            ...
            xhr.setCallbackSuccess(function () {
                ...
                widgets.policy.getBody().removeTR(key);
    ...
    if (window.page.checkPolicy('iam/policy/action/delete')) {
    ...
    let infinite_setting = widgets.policy.getSetting();
    ...
    if (infinite_setting !== null && window.page.checkPolicy('iam/user/action/save/widget/setting')) {
        ...
        infinite_setting.setHardcode('widget', 'policy');
