(function (window) {
    ...
    let url = '/api/iam/user/detail'
        ...
        + String.fromCharCode(61)
        + Date.now();
    widgets.user_detail = new Info();
    widgets.user_detail.setUrl(url);
    window.elements.grid.appendChild(widgets.user_detail.out());

    if (window.page.checkPolicy('iam/user/action/assignment')) {
        let api = '/api/iam/user/assignment'
            + String.fromCharCode(47)
            + encodeURIComponent(window.reference[0])
            + String.fromCharCode(63)
            + 'timestamp'
    ...
    window.action.policy.reassignment = {
        ...
        function: function (event) {
            ...
            let xhr = new XMLHttpRequest(), api = '/api/iam/policy/set'
                + String.fromCharCode(47)
            ...
            data.append(AssignmentLine.id(), 'User' + '/' + window.reference[0]);
    ...
    for (let item = 0; item < window.page.tables.user.fields.length; item++) widgets.form.addInput(window.page.tables.user.fields[item]);
    ...
    window.action.detach = {
        ...
        function: function (event) {
            ...
            let xhr = new XMLHttpRequest(), detach_url = '/api/iam/user/detach'
                + String.fromCharCode(47)
                + encodeURIComponent(window.reference[0])
                + String.fromCharCode(63)
                + 'timestamp'
    ...
    if (window.page.checkPolicy('iam/user/action/policies')) {
        let api = '/api/iam/user/policies'
            + String.fromCharCode(47)
            + encodeURIComponent(window.reference[0])
            + String.fromCharCode(63)
            + 'timestamp'
    ...
    if (window.page.checkPolicy('iam/user/action/assignment')) {
        ...
        widgets.sidepanel.setActionShow(function (event) {
            ...
            window.edit.setText(window.page.getTranslate('sidepanel.edit_off'));
    ...
    if (window.page.checkPolicy('iam/user/action/assignment')) widgets.dropper.setSuccess(function () {
        widgets.list.request();
    });

    widgets.user_detail.request(function (json) {
        if (json.status !== true
        ...
        let line_email = new Info.Line(this),
            line_role = new Info.Line(this);

        let text_mail = null === json.data.email ? String.fromCharCode(45) : json.data.email,
            text_role = null === json.data.role ? String.fromCharCode(45) : json.data.role;

        line_email.getContainer().appendChild(this.constructor.getIcon('material-icons email'));
        line_email.getContainer().appendChild(document.createTextNode(text_mail));
        line_role.getContainer().appendChild(this.constructor.getIcon('material-icons engineering'));
        line_role.getContainer().appendChild(document.createTextNode(text_role));

        this.getLine().appendChild(line_email.out());
        this.getLine().appendChild(line_role.out());

        if (null !== json.data.language) {
            let line_nationality = new Info.Line(this);
            let icon = widgets.form.findContainer('language').getPlugin().getLi(json.data.language).getIcon().out();

            let text_nationality = window.page.getTranslate('info.phrase');

            let container = document.createElement('div');
            container.className = 'nationality';
            container.appendChild(document.createTextNode(text_nationality));
            container.appendChild(icon);
            line_nationality.getContainer().appendChild(container);
            this.getLine().appendChild(line_nationality.out());
        }
    ...
    if (window.page.checkPolicy('iam/user/action/assignment')) widgets.search.request();
